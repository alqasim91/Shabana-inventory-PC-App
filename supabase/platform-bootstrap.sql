-- Supabase platform bootstrap — vendored, not hand-written.
--
-- SECURITY/CORRECTNESS-CRITICAL — flagged in BUILD_PLAN.md task "Source
-- Supabase platform bootstrap SQL" for a dedicated review pass before this
-- ships to a real customer. Runs once, on a fresh database, before any of
-- the app's own 32 migrations. Getting a role/grant wrong here doesn't
-- throw a visible error later — it silently changes what RLS enforces.
--
-- Source: github.com/supabase/postgres, path
--   migrations/db/init-scripts/{00000000000000-initial-schema,
--   00000000000001-auth-schema, 00000000000002-storage-schema,
--   00000000000003-post-setup}.sql
-- Fetched at commit ff09b101523c6479f37fe1e0d02c5f7e3845104c (2026-08-09),
-- concatenated in filename order — this IS the order Supabase's own
-- Postgres Docker image applies them in.
--
-- These are the scripts Supabase's `postgres` Docker image runs before a
-- customer's own migrations, on every self-host/cloud project. They are
-- deliberately NOT rewritten from memory here — vendored verbatim (below
-- the shim block) so the diff against upstream stays auditable, and so a
-- future re-sync is a re-fetch + re-diff, not a re-derivation.
--
-- One deliberate addition, not present upstream: in Supabase's own Docker
-- setup, `supabase_admin` is created implicitly, as the initial superuser
-- Postgres itself is initialized with (POSTGRES_USER=supabase_admin passed
...
-- actually sets (older/newer PostgREST versions have used
-- request.jwt.claims (plural, JSON) instead of per-field GUCs). This is
-- exactly the kind of detail a review pass should re-check against the
-- pinned PostgREST version in .github/workflows/build-installer.yml
-- rather than trust unread.
...
-- ============================================================
-- Not currently used — see BUILD_PLAN.md "Attachments": local installs
-- are not running the Supabase Storage server. Left in, inert, for
-- forward-compatibility rather than stripped out; costs nothing unused.
...
-- 00000000000003-post-setup.sql
-- ============================================================
-- The pg_cron/pg_net event triggers below are no-ops here: this installer
-- never vendors those extensions, so their `IF EXISTS`/`IF schema_is_cron`
-- guards never fire. Left in verbatim rather than stripped, so this file
-- stays a straight diff against upstream instead of a hand-edited fork.
